...
def compute_eia_impact(raw_data: dict, seg: str) -> tuple:
    ...
    for series_id, series_info in eia_data.items():
        ...
        # NG storage: tighter thresholds, larger impact on EIA multiplier
...
def run_pipeline():
    ...
    try:
        ...
        # Dynamic risk-free rate from FRED (FEDFUNDS is in %, e.g. 5.25)
        fred_ffr = raw_data.get("fred", {}).get("fed_funds_rate", RISK_FREE_RATE * 100)
        risk_free_rate = fred_ffr / 100.0
        if not (0.001 < risk_free_rate < 0.15):
        ...
        for seg in qualifying_segments:
            ...
            seg_tickers = [t for t in seg_cfg.get("tickers", []) if t]
            if not seg_tickers:
                continue

            # Segment-level signals (shared across all tickers in this segment)
            ...
            for ticker in seg_tickers:
                ...
                if spot <= 0:
                    print(f"  WARNING: No spot price for {ticker} ({seg}), skipping")
                    continue
                ...
                if not chains:
                    print(f"  [{seg}] {ticker}: keine Options-Chain verfügbar, skipping")
                    continue
                ...
                for opt in relevant_chains[:MAX_CANDIDATES_PER_SEGMENT]:
                    try:
                        ...
                        print(f"  [{seg}/{ticker}] candidate error: {e}")
                        continue

                print(f"  [{seg}] {ticker}: {len(relevant_chains[:MAX_CANDIDATES_PER_SEGMENT])} geprüft → {accepted} Kandidaten nach Filtern")

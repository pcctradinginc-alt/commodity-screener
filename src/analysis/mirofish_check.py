...
JETZT MIT MERTON JUMP-DIFFUSION + REGIME-SWITCHING + datetime Fix
"""
...
class MirofishChecker:
    ...
    def _is_release_regime(self, dte):
        """Einfache Regime-Erkennung für EIA/COT-Tage"""
        today = datetime.date.today()
        future_weekday = (today.weekday() + dte) % 7
        is_eia_day = future_weekday == 2      # Mittwoch
        is_cot_day = future_weekday == 4      # Freitag
    ...
    def _simulate_one(self, candidate):
        ...
        jumps = rng.poisson(jump_prob, size=(N_PATHS, n_days))

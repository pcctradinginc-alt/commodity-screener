...
class DataFetcher:
    ...
    def fetch_tradier_quotes(self):
        ...
        # Tradier supports comma-separated symbols in one request
        symbols = ",".join(all_tickers)
        url = f"https://api.tradier.com/v1/markets/quotes?symbols={symbols}"
        try:
            r = self.session.get(url, headers=headers, timeout=15)
            if r.status_code == 200:
                data = r.json()
                quotes = data.get("quotes", {}).get("quote", {})
                # Single ticker → dict; multiple tickers → list
    ...
    def fetch_options_chains(self):
        ...
        for seg_cfg in self.cfg.get("watchlist", {}).values():
            for ticker in seg_cfg.get("tickers", []):
    ...
    def fetch_cot_data(self):
        ...
        cot_data = {}
        seen_codes: dict = {}  # cot_code → result (avoid duplicate fetches for same contract)
        for seg, seg_cfg in self.cfg.get("watchlist", {}).items():
            cot_code = seg_cfg.get("cot_code", "")
            if cot_code and cot_code in seen_codes:
                result = seen_codes[cot_code]
            else:
                result = analyzer.get_cot_data(seg_cfg["tickers"][0], seg_cfg)
                if cot_code:
                    seen_codes[cot_code] = result
            for ticker in seg_cfg.get("tickers", []):
                if ticker not in cot_data:
                    cot_data[ticker] = result
                    print(f"  [COT] {ticker} → {result.get('signal_strength')} | z={result.get('z_score',0):.2f}")
        return cot_data
